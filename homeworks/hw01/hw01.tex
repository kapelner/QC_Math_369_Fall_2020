...
\hardsubproblem{[MA] Consider the scenario in 1(k). Sample subpopulation A with size $n_A$, sample subpopulation B with size $n_B$, sample subpopulation C with size $n_C$ and sample subpopulation D with size $n_D$. Create an unbiased estimator for $\theta$ using these four samples (and prove that it is unbiased).}\spc{6}

\easysubproblem{If the DGP is $\Xoneton \iid \bernoulli{\theta}$ and we choose the estimator $\thetahat_{\text{bad}} = 1/2$. Why is this estimator a bad choice?} \spc{3}
...
\hardsubproblem{Compare $\thetahat_{\text{bad}}$ to $\Xbar$ using the sup risk. How much better is $\Xbar$? With $n$ getting larger does it get better?}\spc{2}
...
\easysubproblem{Regardless of how you do your convincing, why is it fundamentally impossible to prove your theory in our context of statistical inference?}\spc{3}
