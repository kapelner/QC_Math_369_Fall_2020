...
\easysubproblem{Run the test and write your conclusion using and English sentence.}\spc{1}
...
\problem{In lecture 5, we did a \emph{two-sided one sample z test}. Using the same data: $n=10$ and $\xbar = 70.5$, we will now test if the population that this sample was drawn from has a \emph{greater} mean than female height at $\alpha = 5\%$. According to \href{https://www.usablestats.com/lessons/normal}{this article}, female height is $\sim \normnot{65}{3.5^2}$.}
...
\intermediatesubproblem{Compute the retainment region and rejection region (remember $\Theta = \reals$) and denote these two regions in your illustration in (b). To compute these regions, I'll provide you with the following fact: $\Phi(-1.645) = 5\%$.}\spc{3}
