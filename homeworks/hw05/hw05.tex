...
To do this, you will need the following theorem. For constants $c, d$ and r.v. sequences $A_n$ and $B_n$ where $A_n \convp a$ and $B_n \convp b$ then $cA_n + dB_n \convp ca + db$. Then you use the \href{https://en.wikipedia.org/wiki/Continuous_mapping_theorem}{continuous mapping theorem} (what we've been calling Thm 5.5.4) and Slutsky's (i.e. follow HW4 1e exactly). Remember $S_1 \convp \sigma_1$ and $S_2 \convp \sigma_2$.}\spc{5}

\easysubproblem{For the same obesity study as in 1(b), now consider the outcome metric \qu{binge episodes per week} which is a count metric (e.g. 1 day, 2 days, 3 days, ..., all 7 days!). Since it's real data, no moments are known! Explain why the normality assumption does not hold for the DGP in both samples.}\spc{3}
...
\intermediatesubproblem{Assume a DGP of $\iid \text{Gumbel}(\theta, 1)$. In HW 4 you proved that $\thetahatmle = \natlog{n / \sum_{i=1}^n e^{-X_i}}$ and $I(\theta) = 1$ (the $I(\theta) = e^{-2\theta}$ in lecture 12 was my mistake so please ignore that). Provide the asymptotic distribution of $\thetahatmle$ and run the test from lecture 12 again given the data there as a 2-sided test.}\spc{7}
